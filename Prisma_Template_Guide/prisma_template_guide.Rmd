...
# 1. ENSURE RETICULATE IS INSTALLED AND LOADED
...
py_install(packages = required_packages, pip = TRUE)

# 3. ROBUST NLTK DATA DOWNLOAD (using R's downloader)
cat("Checking for NLTK 'wordnet' data...\n")
tryCatch({
  # Find the primary path where NLTK stores data
  nltk <- import("nltk")
  ...
  # Check if the file is already there. If not, download it using R.
  if (!file.exists(wordnet_zip)) {
    cat("-> 'wordnet.zip' not found. Downloading via R...\n")
    
    # Create the 'corpora' directory if it doesn't exist
    ...
    # Download the file using R's built-in downloader
  ...
  # Final verification by Python to be safe
  nltk$data$find('corpora/wordnet.zip')
  cat("-> NLTK 'wordnet' data is ready.\n")
  
}, error = function(e) {
  stop(paste("A critical error occurred while trying to secure NLTK data. Please check your internet connection and firewall settings. Error:", e$message))
})


# 4. SOURCE THE PYTHON SCRIPT AND CALL THE FUNCTION DIRECTLY
source_python("./auxillary_scripts/similar_words.py")

# NOTE: Make sure the `prelim_keywords` variable is defined before this chunk runs!
...
### Tracking Your Steps
As you proceed though these next sections, studies will be removed from your search based on critera you set. Even then, we still need to track everything. As stated earlier in this tutorial, all decisions are fine, but everything needs to be tracked and justifiable.
