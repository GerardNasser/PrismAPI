...
![PRISMA Flowchart showing what information we need to record.]("./prisma_example_flowchart.jpg")
...
library(dplyr)
output_of_dedupe <- "combined_database_results-deduped.csv"
tryCatch({
  input_of_results <- utils::read.csv("combined_database_results.csv", stringsAsFactors = FALSE)

  # Step 2: Deduplicate based on lowercase 'title' and 'year'.
  # We use group_by() to consider each combination of lowercase title and year.
  # Then, slice(1) keeps only the first row for each of those unique groups.
  # The original capitalization from that first row is preserved.
  ...
  # Step 3: Save the deduplicated data to a new CSV file.
  # Replace 'your_output_file.csv' with your desired output file name.
  write.csv(deduplicated_df_dplyr, output_of_dedupe, row.names = FALSE)

  deduplicated_total <- nrow(deduplicated_df_dplyr)
  
  # Print a success message to the console
  print("Deduplication using dplyr is complete. Check 'your_output_file_dplyr.csv'.")
  print("Original rows:")
